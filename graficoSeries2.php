...
if(isset($_POST["top"])){
  ...
  // $SQL1="SELECT m.title,m.Registered, from movies m where m.estado = 1  and m.Registered>='".$fechamin."' and m.Registered<='".$fechamax."' order by m.Registered asc ".$Limit ;
    $SQL1="SELECT registered, COUNT(Registered) from series where Registered>='".$fechamin."' and Registered<='".$fechamax."' group by Registered asc".$Limit ;
...
//            }
    ...
    title: {
              text: 'REPORTE GENERADO EL <?php date_default_timezone_set('america/lima'); $fecha_actual = date("d-m-Y"); echo $fecha_actual." a las ".date("H:i:s")."<br>" ; ?> <?php if($TOP!="all"){echo "TOP ".$TOP;} ?> Series regisradas <?php if($fechamin!="1999/01/01" || $fechamax!="2090/01/01" ){echo "desde ".$fechaminString." hasta ".$fechamaxString; } ?>'
            },
    xAxis: {
        type: 'category',
        labels: {
