...
<div class="panel-heading" style="text-align:center;"><h2>Grafico de Foros</h2></div>
...
<div class="form-group">
    ...
    <button class="btn btn-info" block="true" type="submit" name="action" value="create"> Generar Reporte </button>
        ...
        <a class='btn btn-danger  '  href='listaforos2.php'>Volver</a>
    </div>
</div>
...
if(isset($_POST["top"])){
  ...
  $SQL1="SELECT f.title,f.respuestas from foro f inner join users u on f.user_id=u.user_id where f.estado = 1  and f.fecha>='".$fechamin."' and f.fecha<='".$fechamax."' order by f.respuestas desc ".$Limit ;
...
//            }
    ...
    title: {
              text: 'REPORTE GENERADO EL <?php date_default_timezone_set('america/lima'); $fecha_actual = date("d-m-Y"); echo $fecha_actual." a las ".date("H:i:s")."<br>" ; ?> <?php if($TOP!="all"){echo "TOP ".$TOP;} ?> Foros más visitados <?php if($fechamin!="1999/01/01" || $fechamax!="2090/01/01" ){echo "desde ".$fechaminString." hasta ".$fechamaxString; } ?>'
            },
    xAxis: {
        type: 'category',
        labels: {
